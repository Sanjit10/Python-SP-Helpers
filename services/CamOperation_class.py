...
import random
from ctypes import *
from services.yolo import YOLOObjectDetector
...
# Convert Mono image to numpy array
...
def Color_numpy(data, nWidth, nHeight):
    ...
    # Debug: Print the actual size of the data buffer
    print(f"Actual buffer size: {len(data)}, Expected size: {nWidth * nHeight * 3}")
...
class CameraOperation:
    ...
    def __init__(self, obj_cam, st_device_list, n_connect_num=0, b_open_device=False, b_start_grabbing=False,
        ...
        self.gain = gain
        self.image_for_yolo = None

        cooked_state_manager = CookedStateManager()
        event_bus = EventBus()
        model_path = str(r'C:\crimson_tech\archi\models\best_100.pt')

        self.model_algo = YOLOObjectDetector(
            model=model_path,
            cooked_state_manager=cooked_state_manager,
            event_bus=event_bus
        )
        self.buf_lock = threading.Lock()  # Buffer lock for capturing and saving images
    ...
    def Open_device(self):
        if not self.b_open_device:
            ...
            # Detect the optimal packet size (only for GigE camera)
    ...
    def Work_thread(self, winHandle):
        ...
        while True:
            ...
            if 0 == ret:
                # Copy image and image information
                ...
                # Convert buffer to a format suitable for YOLO model
                if not self.convert_to_numpy_array():
                    continue
                # Run YOLO model
                if self.image_for_yolo is None:
                    print("image_for_yolo is None")
                    continue
                detection_results = self.run_yolo(self.image_for_yolo)          
            else:
    ...
    def display_detection_results(self, detection_results):
        nHeight, nWidth, _channels = detection_results.shape
        detection_results_pointer = detection_results.ctypes.data_as(ctypes.POINTER(ctypes.c_ubyte))
        self.st_frame_info.nWidth,self.st_frame_info.nHeight = nWidth, nHeight 
        self.st_frame_info.nFrameLen = nWidth * nHeight

        # Display image using Display interface
        ...
        stDisplayParam.enPixelType = PixelType_Gvsp_RGB8_Packed
        stDisplayParam.pData = detection_results_pointer
    ...
    def convert_to_numpy_array(self):
        ...
        self.image_for_yolo = numArray
        return True

    def run_yolo(self, image):
        print(type(image))
        return self.model_algo.object_detection_yolo(image)
